...
export default function CreateSocialPostScreen({ route, navigation }: any) {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      ...
      setBgIndex(0); // FB doesn't allow BG if image is present
    }
  };
      ...
      <Box pt={insets.top + 10} pb={12} bg="white" borderBottom={1} borderColor="#E5E7EB">
         <HStack items="center" justify="space-between" px={16}>
            <HStack items="center">
               ...
               <Text fontSize={18} fontWeight="700" color="#1C1E21" ml={16}>Create Post</Text>
            </HStack>
            <Button 
               label={loading ? '...' : 'Post'} 
               onPress={handlePost}
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
         <Box p={16}>
            <HStack items="center" mb={16}>
               ...
               <VStack ml={12}>
                  <Text fontSize={16} fontWeight="800" color="#1C1E21">{user?.name || 'User'}</Text>
                  <HStack space="sm" mt={4}>
                     ...
                     <TouchableOpacity style={styles.audienceSelector}>
                        ...
                        <Text fontSize={11} fontWeight="800" color="#65676B" ml={4}>Friends</Text>
                        <ChevronDown size={11} color="#65676B" style={{ marginLeft: 3 }} />
                     </TouchableOpacity>
                  </HStack>
               </VStack>
         ...
         {/* Text Background Selector (Only if no image) */}
         {!image && (
         ...
         {/* Bottom Action Sheet (Persistent part) */}
         <Box borderTop={1} borderColor="#E5E7EB" bg="white" style={{ marginTop: 20 }}>
            ...
            <VStack>
               <ToolOption icon={ImageIcon} label="Photo/video" color="#45BD62" onPress={pickImage} />
               <ToolOption icon={Users} label="Tag people" color="#1877F2" />
               <ToolOption icon={Smile} label="Feeling/activity" color="#F7B928" />
               <ToolOption icon={MapPin} label="Check in" color="#F5533D" />
               <ToolOption icon={Video} label="Live video" color="#F02849" />
            </VStack>
         </Box>
      </ScrollView>
    </ScreenWrapper>
