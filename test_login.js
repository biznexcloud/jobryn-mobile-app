...
async function testLogin() {
  ...
  let output = "=== Testing Login API ===\n";
  output += "URL: https://backend.jobryn.com/api/v1/account/login/\n";
  output += "Payload: " + JSON.stringify(payload, null, 2) + "\n\n";
  
  try {
    const response = await axios.post('https://backend.jobryn.com/api/v1/account/login/', payload, {
      headers: {
  ...
  console.log("Done - check test_login_output.txt");
}
